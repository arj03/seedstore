...
This is the design spec. For an overview and a quick start, see the [README](../README.md). For the implementation of Part I, see [WASM/README.md](../WASM/README.md).

Seedkernel routes a wire protocol id to one bundle's confined guest over an encrypted, authenticated channel. It has no notion of a file and nowhere for a multi-megabyte one to live. Seedstore is that store, built only from pieces seedkernel already provides: a channel that pins who is speaking, a claim to dispatch on, and a signed bundle that carries the code and its authority. Bulk bytes live beside the dispatch path, referenced by content hash (§3).

**Goals.** A node donates whatever storage it has and stores files across its peers such that:

- no single peer can make a file unavailable (§10);
- peers can be offline for stretches without data loss (§8);
- the system heals itself when redundancy drops (§9);
- file size is bounded only by the cohort, not by any message or object size (§4.3);
- good citizens earn durability for their own data through direct reciprocity, not a coin (§13).

**The assumption.** The network is **closed and social**. You store with peers you have a relationship with (friends, friends-of-friends, or an explicit storage group), not in an open market of strangers. That assumption is what keeps the design small: privacy and Sybil resistance come from the *shape* of the network, not from added cryptographic machinery. The whole system is five ideas: **a social cohort, client-side encryption, content addressing, erasure coding, and a have/want exchange**.

**Design principles:**

- **No new host concepts.** Storage is one signed bundle (pure-transform WASM modules plus a confined guest) and four request/response messages (§18). The channel that already pins every peer and the policy that already admits every bundle gate it.
- **Integrity comes from content addressing, not signatures.** A block either hashes to its name or is discarded, so bulk transfer needs no per-message verification (§3).
- **Identity comes from the channel.** A peer *is* the node public key its authenticated channel pinned (§2). Nothing is signed to establish it.
- **Redundancy is erasure coding, not replication.** Any *k* of *n* blocks reconstruct a chunk (§4.1).
- **Placement is by relationship, not global address.** Blocks live on cohort peers chosen by negotiation, and who holds what is discovered live. There is no global index, by design (§5).
- **Confidentiality is structural.** The wire is encrypted, stored data is encrypted so holders see only ciphertext (§4.4), and no directory maps content to holders.
- **Trust is local before it is global.** Direct, pairwise reciprocity (§13) is the baseline. Portable reputation is an optional layer (§20).
- **The trusted base is small and separate from the logic.** Only the host's I/O backends are trusted. All storage logic is confined and upgradeable (§2.1).
- **Browser nodes and long-running peers run the same protocol.** They differ only in their `store` backend and default quota (§12).

---

## 1.1 Concepts at a glance

- **Block** — the one unit of stored data: a content-addressed slice of ciphertext of `B` bytes (block size `B` is a deployment tuning knob, §28, kept small so a whole block re-fetches cheaply, §8; no protocol constant bounds it — a transport's byte cap is operator policy, §3). `block_id = block_hash(author_pk, block_bytes)`. A block is simultaneously an erasure-coding shard *and* the unit that moves on the wire.
...
- **Root** — what you are handed to read a file, and all you need: one signed chunk descriptor plus the content key `K` (§4.4). **There is no manifest object** (§4.3) — a file too big for one chunk has an *index*, which is nothing but the list of its descriptors, chunked and placed like any other bytes, and the root is the descriptor of that. Neither names holders; which peer holds a block is discovered live from the cohort.
...
- **`store`** — the storage layer's own facade over the host's flat-key `fs` service (§16), whose backend reads/writes the node's donated bytes: filesystem on a server, OPFS/IndexedDB in a browser (§12).
...
## 2. How it composes with seedkernel

Nothing here changes `install`, the guest seam, or claim routing (seedkernel §12.10). Storage shows up as three kinds of seedkernel object:

**The channel — which is also the identity.** Seedstore needs an authenticated channel between peers, and seedkernel's node↔node transport is exactly that on every byte-pipe (TCP, WebSocket, WebRTC): an authenticated key exchange, after which every frame is a forward-secret, individually-authenticated encrypted record attributed to the peer's key ([seedkernel §12.6](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md), [CHANNEL](https://github.com/arj03/seedkernel/blob/main/docs/CHANNEL.md)). That channel **is** storage's identity layer, and there is nothing else to it: a peer *is* the node public key its channel pinned. Every operation that needs "who" — who is offering, who is fetching, whose reciprocity standing moves — reads it off the channel rather than out of a signature, so there is no per-message signing and no separate account system. Storage reaches its peer RPC through the transport bundle's ordinary local service id `_net` — a claim under the transport's `services`, no privilege of its own, addressed by the storage bundle's signed `protocols` id (§12.10): a frame for that id lands in the storage slot, and only the storage slot's guest can see it. The live peer RPC is typed request/response on that channel (§3, §18), not module dispatch, and bulk blocks ride the same frame plane as hash-verified payloads of ordinary storage messages (§3, §18). A dedicated bulk channel is an optional performance upgrade (§22).

**The bundle — which carries the code, its names, and its authority.** Storage arrives as one author-signed bundle (seedkernel [§12.4](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md)), and that one signature is what authenticates storage's code to a node that may have received it through any number of relays — the one place signing genuinely survives. The bundle holds the pure-transform WASM modules (`codec`, `reputation`), declared in the manifest's signed `modules[]` and reached by the guest by their **bare name** — `host.call("codec", …)`. There is no bind name and no global namespace: a module is a private value of its own slot, and the seam resolves a bare name against that slot's map (seedkernel §5, §12.2), so nothing on the wire ever names another node's module. It also holds the confined guest program and, in the same signed manifest, the *reach* that guest may use (seedkernel §12.4), in one explicit, human-auditable list declared by the author (`guest.requires` — `node`, `fs`, `_net`): the host services it is granted, gated **by service** by the host, and the co-resident guest it calls, the network. The pure modules — the codec (chunking, erasure coding) and the reputation math — hold **no** grants at all: they are computation, and the structural sandbox guarantees they can touch no I/O even if compromised. Deployments are kept apart not by names but by policy and scope: the admission policy's closed author set decides whose bundle may install (seedkernel [§12.5](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md)), and the scoped signature (§16) keeps one deployment's signed objects from verifying in another's.

**Crypto — reused, never shipped.** Storage reuses the host's cryptography rather than bundling its own, so the only cryptographic-grade algorithm in storage WASM is **Reed–Solomon** (libsodium has no erasure coding); hashing and ChaCha20-Poly1305 use the host's residual ungated `crypto/*` transforms, while key-sealing is deliberately host-side over the same core libsodium (§16). A chunk descriptor is signed and checked through the host's *scoped* signing pair, and its detached authentication tag travels under that signature. Confidentiality *at rest*, which the channel's hop-by-hop encryption does not give (seedkernel §14), is therefore added client-side with no second crypto library (§4.4).

What the host deliberately does **not** give storage is anywhere for the bytes themselves to live: a module is a pure transform over a scratch buffer, and the channel moves opaque frames. Bulk data therefore lives beside the dispatch path, referenced by hash — the split §3 treats first.

---

## 2.1 Trust model: three tiers

Running a seedstore node extends your trusted computing base (TCB) — but the design keeps the part you *must* trust small and separate from the part that changes. Three tiers, smallest-TCB-first:

**Tier 0 — host base.** seedkernel itself: the host, whose `install` admits code under an admission policy. The root of trust; everything else composes on top.

**Tier 1 — I/O base (trusted).** The host services' raw-byte backends — the flat-key `fs`, the `node` authority (scoped sign/verify), and the raw `link` seam the transport bundle owns — plus the residual ungated `crypto/*` transforms over core libsodium (seedkernel §12.1). These *must* be trusted: they are the only code that performs real I/O. They are deliberately small, each bound to exactly one service, audited, and change rarely. This is the *whole* of what seedstore adds to your TCB.

**Tier 2 — logic (confined, upgradeable).** Everything else: `codec`, `reputation`, and the `coordinator`/`cohort`/`repair` orchestration (§17). This is pure logic. It is **confined** — a WASM module imports nothing, a realm has no ambient authority — and reaches I/O *only* through the Tier-1 services its manifest's `guest.requires` names. The grant is signed content, not something the logic could declare for itself: the author names the services in the signed bundle manifest (§2, seedkernel §12.4), and the host wires only those backends. So upgrading the logic does *not* re-grant trust: a new version (same author, seedkernel §12.5) runs in the same sandbox, structurally bounded to whatever its manifest grants — and because the grant is signed, versioned content rather than ambient authority, a widened grant is a visible manifest change, never a silent acquisition.

Two implementation forms satisfy Tier 2, both confined identically:

- **WASM** — the pure, hot compute (`codec`, `reputation`). The wasm sandbox is the boundary; admission and same-author upgrades are `install`'s job under its admission policy (seedkernel §12.4–§12.5) — an upgrade is not a separate mechanism, just a bundle whose manifest `version` is higher, signed by the author already recorded at the slot. A module holds no grants at all — pure computation with no I/O reach to govern.
...
The line that matters: **"trusting seedstore" means trusting Tier 1 (small, stable, audited) — not Tier 2 (large, churning, upgradeable).** A buggy or compromised Tier-2 unit, of either form, can do nothing but compute and call the services it was granted; it can never reach I/O outside that grant, and an upgrade cannot silently widen it — a wider grant is a visible change to the signed manifest.

> **In the [seedkernel](https://github.com/arj03/seedkernel) runtime** these tiers map directly onto its [RUNTIME §12](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md): Tier 1 is the host services' raw-byte backends behind the one guest seam (§12.1–§12.2); Tier 2 is the seedstore **bundle** — `codec` and `reputation` as no-grant WASM modules, the orchestration and holder side as zero-authority JS in its one realm (§12.3) — delivered as signed content (§12.4). A node *is* the generic host plus this bundle; nothing storage-specific lives in the trusted runtime. See the [WASM implementation README](../WASM/README.md) for how to run one.

---

## 3. The two planes: control and bulk

The host gives bulk bytes nowhere to live (§2), so they live beside the dispatch path, referenced by hash. That is the split, and the two halves are worth separating for a second reason: **they authenticate differently.**

**Control plane — small peer-to-peer messages.** Root descriptors, have/want exchanges, placement offers, fetch requests, repair coordination. The storage RPC between two cohort peers (§18) rides the seedkernel transport's authenticated channel and needs **no per-message signature**: the channel pins the remote's node identity and encrypts, integrity-protects, and replay-protects every frame itself (seedkernel §12.6), so the channel is the sender authentication.
...
**Bulk plane — content-addressed blocks.** Blocks are **self-verifying by hash**, so they need no signature for integrity. In the base design they ride *inside* the same channel's messages — the push half of a `store` and the reply half of a `fetch` (§18) — as unsigned, hash-named bytes: the receiver verifies `block_hash(author_pk, bytes) == block_id` against an id it asked for or admitted, and drops on mismatch — the hash, never the framing, is the authority. This stays entirely inside the seedkernel transport, inherits the channel's encryption and pubkey pinning, and avoids per-block verify cost because there is no signature to check. (A dedicated bulk data channel with its own raw framing, for higher throughput on large files, is an optional upgrade — §22.)
...
Each block is content-addressed: `block_id = block_hash(author_pk, block_bytes)`, where

```
block_hash(author_pk, bytes) = BLAKE2b-256(ASCII("seedstore:block\0") ∥ author_pk[32] ∥ bytes)
```

Content addressing makes every block **self-verifying**: a receiver recomputes the hash and rejects anything that doesn't match, so a malicious holder cannot return corrupt bytes undetected, and no signature is needed on bulk data (§3). Because the bytes are ciphertext (§4.4), a `block_id` is the hash of an encrypted blob — opaque and unguessable to anyone who has not handled that exact file.

The **author is inside the hash**, taken from the signed descriptor envelope that carries the block (§4.3) — the zero byte terminates the domain string, and the key is a fixed 32 bytes, so the three fields cannot be slid past one another. Hashing the bytes alone would name a block by content only, and a cohort peer could then re-sign another author's ciphertext under its own key, keep every id intact, and substitute its own sibling list and geometry: the signature check passes, the ids still match, and the chunk is quietly re-parented. Binding the id to the author makes that a different id, so re-signing forfeits the blocks. Being a known peer (§5.1) buys admission eligibility, never ownership of another peer's blocks. Holders, readers and repairers all recompute the same way, from the author's *public* key alone — keyless repair (§9) is untouched.
...
The descriptor carries no chunk index: a chunk's position in the file is given by its order in the index level that lists it, and a block-holder repairing a chunk never needs the position — only the sibling block-ids and the chunk's own `(k, m, B)` — so those, unique-by-content, are the whole identity. The same descriptor object thus lives in **two homes**: inside the (encrypted) index, so a reader gets every chunk's shape at once; and in the clear alongside each stored block, so a repairer who lacks the index still has its chunk's shape and can verify it from the author's public key alone. Both homes are load-bearing for the signature, for different reasons: a holder checks it at *admission*, where storage authority is actually being granted, and a reader checks it on the *root* alone (§7), which is the one descriptor with no authenticated parent — every descriptor below it is reached through content-addressed ids and needs no second check. It is small and signed, so duplicating it is cheap and tamper-evident in both places.

### 4.4 Encryption (the load-bearing privacy mechanism)

The seedkernel channel encrypts a link hop-by-hop, but nothing it carries stays confidential once written to a holder's disk. In this design, **encryption is what makes the closed network safe** — it lets you store on cohort peers who can read nothing, and it makes block-ids opaque. Seedstore encrypts **client-side before erasure coding** through the host's existing ChaCha20-Poly1305 transform (§2, §16), not bundled crypto. Ciphertext stays length-preserving for erasure coding; the 16-byte authentication tag travels separately inside the author-signed chunk descriptor, so every stored block remains exactly `B` bytes and the systematic data blocks remain ciphertext verbatim:
...
- **Sharing a file is sharing the key, not moving bytes.** The owner sends a recipient `{ root_descriptor, seal(K → recipient_pubkey) }` over the authenticated channel — `K` sealed to the recipient's node public key (libsodium `crypto_box_seal`, converting the Ed25519 node key to X25519). The key is never stored in clear on holders, which is what avoids the circularity of putting `K` inside an index that `K` encrypts. Re-sharing is one more sealed copy; revocation that must deny future reads rotates `K` and re-encrypts (§23).
...
1. **Chunk & encrypt.** The owner generates a random content key `K` (via `crypto/random`, §16) and feeds the file to the `codec` (no grants) block-by-block, encrypting each chunk under `K` (§4.4).
...
1. **Resolve the root.** You were handed the root descriptor and the sealed `K`. If its signed `level` is 0 it already describes file bytes and you are done. Otherwise read that chunk by steps 2–4 below — the same fetch, verify, decode, decrypt — and its plaintext *is* the descriptor list one level down; repeat, which in practice means once. **Each step must strictly decrease `level`**, and a reader enforces that rather than assuming it: whoever supplied `(root, K)` can construct valid AEAD at every level, so authentication alone does not stop a level-ℓ root from naming a level-ℓ list. Without the check a reader can be walked through an arbitrarily long non-descending chain. With it, the descent is bounded by the `level` byte. The file size is the sum of the leaves' signed `tail_bytes`. **The root's author signature is checked before any of it is used** — its `level`, geometry and `tail_bytes` are read before there is anything to hash-check them against, and the AEAD tag covers the chunk's ciphertext, not the descriptor's own fields. Below the root no signature check is needed: every level is reached through content-addressed ids (§4.2), so a tampered index fails its hash before it is parsed.
...
3. **Fetch & verify.** Stream blocks over the bulk plane; each is checked against its `block_id` (self-verifying, §4.2). A response that doesn't parse — a bad status byte, the wrong entry count, truncation, trailing bytes — is that holder's miss (§8), not the read's failure: the reader falls to the next holder, exactly as it would for a block the holder didn't have. The §9 repair audit reads the same way.
...
"Donate whatever storage you have available" is the storage layer's `store` facade over the host's flat-key `fs` service, plus a host-configured quota.

**The `store` facade (§16)** exposes `put(block_id, bytes)`, `get(block_id) → bytes`, `size(block_id) → n | −1` (existence is `size ≥ 0` — there is no separate `has`, mirroring the host's `fs/size`), `delete(block_id)`, `list()`, and `stat() → { quota, used, free }`. It is nothing but the `fs` service's names under storage's own key namespace: like every granted service it exists in a realm only if the bundle manifest declares it (§2.1), so only logic that may use `fs` can reach disk at all. A holder stores opaque `(block_id → ciphertext)` pairs plus the small signed descriptor (§4.3): it needs no file key and learns nothing about what it is holding beyond the chunk's shape.
...
**Residual host-inherited risk.** The host does not bound a module's compute beyond the calling guest's execution budget (seedkernel §4.3), so run the heavy `codec` and `reputation` modules under a Worker watchdog (the Tier-2 sandboxed-JS realm reaches them *through* that budget, §2.1). And the storage guest is the grant-richest Part I unit — the one whose realm holds the most logic, behind the one required set (`node`, `fs`, the transport's `_net`) — so it is the largest blast radius if a storage unit is ever compromised, the prime audit target, and a standing reason the admission policy (§19) stays a closed allowlist rather than an open author set. Tier-2 confinement (§2.1) bounds that blast radius to its granted services: a compromised guest computes and drives those names, nothing more.

---

## 16. What the host gives storage: service grants, primitives, and one shared transport

Storage's confined logic reaches the world through one seam — `host.call(name, bytes)` (seedkernel §12.2) — and the names it relies on are the host's own surface, in three kinds: **granted host services** (real I/O or the node's authority, each the host's own), one **local service id** (`_net`, another bundle's — the transport's — with no privilege of its own), and **ungated names** — the residual `crypto/*` host transforms and the bundle's own module names — which reach nothing a guest could not compute itself and therefore carry no grant at all. Together the granted ones are the whole of Tier 1 (§2.1). They are not storage-specific host objects: the runtime side of the seam, the grants-by-service rule, and the claim routing are the host's own ([RUNTIME §12](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md)); the table is storage's map onto it.

| Name | Kind | What it gives storage |
| --- | --- | --- |
| `node` | granted service | the node's own things: `node/sign`/`node/verify` under **this slot's one scope** (§16's signing paragraph), the node's public key arrives as `HOST.identity` rather than as a call |
...
| `crypto/blake2b`, `crypto/chacha20poly1305-ietf/{seal,open}`, `crypto/random` | ungated host transforms | the §4.2 content hash, §4.4 authenticated encryption, and the entropy for content keys (nonces are derived, not drawn, §4.4) |
...
`_net` is the one entry that is not the host's: it is ordinary claim routing (seedkernel §12.10). The transport is itself a bundle holding the **`link`** service — the one exclusive raw-net seam, held by the transport the node selected at boot or explicitly swapped in (seedkernel §12.5) — showing a structured service under a local service id the storage guest names in its `guest.requires`. Calling `_net` carries no privilege: a co-resident call from one bundle slot to another, reached through the same claim table that routes a peer's protocol id. Storage adds no backend of its own; time is the realm's own `Date.now()` (liveness, repair jitter, score decay) and entropy is `crypto/random`.

**Crypto is reused, not added.** The transforms storage uses are already in the host's residual transform table (seedkernel §12.1), and they perform no I/O, so they need **no grant**. **Why a storage-local hash:** block-ids never cross into the host — they are pure content addressing within storage (§4.2) — so BLAKE2b-256 remains storage's own choice even though the implementation is already present in core libsodium. Key-sealing is deliberately absent from the guest seam, so the one audited use lives beside the guest in `host/crypto.ts` over that same core instance. Storage therefore ships **no crypto of its own except Reed–Solomon**, and dropping the old MAC-less XChaCha20 stream call lets the shared shell use the smaller core libsodium build.

**The sign/verify pair exists — but it is scoped, and every signed format is tagged.** Storage logic authors its signed objects — the chunk descriptor (§4.3) and the optional tombstone (§25) and file head (§27.3) — through the host's scoped pair, never a raw oracle (seedkernel §12.2): `node/sign` signs `DOMAIN_guest ‖ scope ‖ msg` with the scope the slot's own, derived from the admitted bundle's `app` label — never from the caller — and **verification is scoped identically under the same name**, so storage names the verifying key and never reconstructs the prefix. (The raw `crypto/ed25519/verify` primitive stays, ungated like every pure transform — storage never hands it a storage signature.) A storage signature thus verifies only as a storage signature — never as a bundle manifest or a channel handshake, the disjoint members of the host's domain family (seedkernel §16.1, §14) — and never as another app's object, while staying portable across the cohort because every node running storage under the same label derives the same scope, whoever authored the bundle. Checking one still needs only the signer's public key — keyless repair (§9) is untouched. One level down, seedstore applies the host's sub-separation rule to its own vocabulary: each signed storage format opens with a **distinct leading tag** — one for the descriptor, one for the tombstone, one for the head — so a signed object of one type can never be replayed as another. Signing never ships in storage WASM at all — a module is a pure transform with no grants (seedkernel §4.2); what storage holds is the seam's scoped pair, never key material.
...
| **the guest** — `cohort`, `store.coordinator`, `repair` in one realm | sandboxed-JS (zero-authority realm) | `node`, `fs`, `_net` (`guest.requires`) — the whole reach, one signed list | the peer set and connections; have/want, liveness, and the verification-fetch sampling that backs it (§8); PUT/GET incl. placement negotiation and content-key generation (via `crypto/random`); windowed transfer; admission, eviction (§14) and reciprocity accounting (§13); the repair loop (§9); and the holder side (admission, sibling rule, content-addressing, quota, store writes) |
...
On top of the host's own boot, a storage-capable node additionally:

1. Wires the platform backends behind the host services (§16) — the flat-key `fs` on a directory (a server) or OPFS/IndexedDB (a browser), the platform's sockets (`link`, consumed by the transport bundle), the clock and the entropy source — none of it storage-specific.
...
The base bulk plane (§3) carries blocks inside the batched `store`/`fetch` messages — simplest, and entirely inside the seedkernel transport. For higher throughput on large files, a deployment can run a **dedicated bulk data channel** alongside the control channel on the same connection. The control plane negotiates a transfer (block-ids, order, window) over the §18 messages; raw blocks then stream over the bulk channel as bare frames `[claimed block_id ∥ bytes]` — the claimed id a routing label only: the receiver drops any frame whose claimed id it did not request (unsolicited bulk costs nothing, not even a hash), then verifies `block_hash(author_pk, bytes) == claimed id` on the rest and drops on mismatch; the hash, never the label, is the authority. This is the most performant option for large files, at the cost of a second channel to manage. The integrity rule is unchanged either way: every block is validated by `block_hash(author_pk, bytes) == block_id`.
...
**Authority.** A tombstone is honored only when signed by the file's author (the §2 identity) — the same trust root that signs chunk descriptors (§4.3). For a shared file the simple rule is that only the owner's tombstone removes the data; a member who no longer wants it just drops its own copy and stops repairing — it cannot delete for everyone.
...
**Custody, not read access.** This is distinct from sharing a file (§4.4), which seals `K` to a *reader*. A Shamir share grants nothing on its own; it is held *against future recovery*, not to read the file. Each share is sealed to its custodian's node public key (the §4.4 mechanism) so an eavesdropper cannot accumulate shares toward `t`, and each custodian independently decides whether to release — so recovery needs no appointed coordinator, just `t` willing custodians.
...
**An edit is a new root.** A block is named by its hash (§4.2) and a chunk by the signed descriptor over those hashes (§4.3), so nothing in the store can be changed in place: a modified block is simply a *different* block, and a modified file is a *new* root descriptor. Deterministic, content-addressed chunking makes this cheap — **every unchanged chunk keeps its exact block-ids**, so the new index relists blocks that are already placed, already discoverable via have/want (§5), and already repairing (§9); only the **changed** chunks are re-coded and placed (§6 steps 2–4), plus the index chunks on the path from them up to the root. Structural sharing across versions thus falls straight out of content addressing, exactly the way it deduplicates identical chunks within one file. An append, or an overwrite of the same length, touches only the trailing or overlapping chunks; the rest of the file costs nothing.
...
Everything above produces a *new* root; nothing yet expresses **"the current version of this file."** The immutable layer deliberately has no mutable name (§4.3), so tracking the latest version is a separate, genuinely new object — a small **signed head** mapping a stable file identity to its current root:
...
file head (signed by the owner — the §2 identity):
  ...
  root           // the current version's root descriptor (§4.3) — the signed
                 //   object itself, variable-length, not a 32-byte name
  seq            // monotonic per (signer, file_id)
```

Replay protection is the head's own `seq` — the first per-signer sequence anywhere in the design (§17): the highest `seq` from the file's author wins, and a stale or replayed head is simply dropped. The head is small, signed, and replicated/gossiped through the cohort like any other control message; resolving a file becomes one extra hop (head → root) before the §7 GET. **Single-writer is the whole easy case** — your own files across your own devices, or one owner per file — and it is all this extension covers. **Concurrent multi-writer editing is a different problem** the design does not solve: two heads at the same `seq` are a genuine conflict, and reconciling them needs last-writer-wins, a merge policy, or a CRDT over the file's index — out of scope here, and a reason to keep mutable files single-author unless you are prepared to build that.

### 27.4 History and reclamation

Old versions are not deleted — they are simply **no longer pointed at**. Each prior root stays a complete, immutable snapshot for as long as someone keeps it (and its key) and repairs it, so version history is free and tamper-evident: a root names exactly one immutable set of bytes. When a version is abandoned, the blocks **unique** to it — the changed chunks no later version relists — stop being repaired, go cold, and age out as orphans under ordinary eviction (§14); blocks still shared with a live version keep their redundancy because a current index still names them. Prompt reclamation of a superseded version uses the existing toolkit: drop the old key to crypto-shred it (§11), or publish a tombstone (§25) for the blocks no surviving version references. Pinning a specific version — a published release, a signed checkpoint — is just keeping that root and refusing to let it be evicted, the §14 "protect rare blocks" lever applied deliberately.

**The cost, and why it is optional.** Write-once data — content-addressed blobs, immutable media, backup snapshots — needs none of this: it already has everything in "an edit is a new root" and never resolves a head. Mutable files cost a nonce discipline (§27.1) you must not get wrong, optionally content-defined chunking (§27.2), and a new signed pointer (§27.3) with the usual single-writer-vs-concurrent tax. Add it only when files genuinely change in place from the user's point of view; the rest of the system never learns that a root had a predecessor.

---

## 28. Tuning knobs and open questions

- **`(k, m)`, chunk size, and block size `B`** — the durability/overhead dial, set once per deployment (§4.1). Size `(k, m)` against measured cohort churn so the chance of losing more than *m* holders within one repair interval is acceptably small; a per-file override is an extension, not core. `B` sets how many blocks a file becomes, and therefore index and have/want size: larger blocks shrink the index but cost more per verification-fetch and more holder memory, so keep `B` small enough that re-fetching a whole block stays cheap — which is what lets proof-of-retrievability skip Merkle paths (§8, §20). A few tens to a few hundred KB is typical; no protocol constant bounds it (§3).
...
Everything above is expressible as modules, one confined guest, two signed objects, and a restrictive admission policy — i.e. as one ordinary seedkernel bundle. The host never learns what a "file" is; it just keeps routing the protocol id to the slot's guest while the channel pins who is speaking, the bulk bytes never enter its dispatch path, and the core stays five ideas deep: a social cohort, encryption, content addressing, erasure coding, and have/want — with reciprocity, not a coin, rewarding the good citizens.
