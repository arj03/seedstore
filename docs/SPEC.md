...
Seedkernel routes a wire protocol id to one bundle's confined guest over an encrypted, authenticated channel. It has no notion of a file and nowhere for a multi-megabyte one to live. Seed store is that store, built only from pieces seedkernel already provides: a channel that pins who is speaking, a claim to dispatch on, and a signed bundle that carries the code and its authority. Bulk bytes live beside the dispatch path, referenced by content hash (§3).
