...
- **Block** — the one unit of stored data: a content-addressed slice of ciphertext of `B` bytes (block size `B` is a deployment tuning knob, §28, kept small so a whole block re-fetches cheaply, §8; nothing ties it to the 64 KB envelope — bulk blocks aren't enveloped, §3). `block_id = content_hash(block_bytes)`. A block is simultaneously an erasure-coding shard *and* the unit that moves on the wire.
...
- **Sandboxed JS** — the orchestration (`coordinator`/`cohort`/`repair`) *and* the holder side (admission, the sibling rule, content-addressing, quota, the store writes), which are awkward to express as a *synchronous* wasm handler. They run in a **zero-authority JS realm**: a fresh context with *no* ambient authority, into which the host injects *only* the capability-gated bridge surface the logic declared. The realm cannot even name `fs`/`net` — those bindings do not exist in it — and CPU/memory are bounded by the host. It gets the same capability + upgrade-acknowledgement discipline as the wasm tier. The async *initiator* side runs in an Asyncify realm; the *holder* side runs in a separate **synchronous** realm, so a node can answer an incoming request while its own initiator realm is parked mid-`await`.
...
> **Implementation status — done.** All Tier-2 logic now runs confined. `codec` and `reputation` are no-cap WASM handlers; the orchestration **and** the holder side run as zero-authority JS in QuickJS realms, reaching I/O only through a fixed, capability-gated bridge of *generic* primitives (hash, sign/verify — the sign scoped, §16 — stream-xor, net send/request-many/peers, fs get/put/size/…, an installed-handler call, clock). Nothing storage-specific remains in the trusted runtime: seedkernel exposes only those raw-byte capabilities and runs seed store as **signed content** — a node *is* the generic [seedkernel](https://github.com/arj03/seedkernel) shell plus the seed store bundle. See the [WASM implementation README](WASM/README.md) for the architecture and how to run a node from the command line.
...
> **Implementation status (§2.1) — done.** `codec` and `reputation` are no-cap WASM. The `cohort`/`store.coordinator`/`repair` orchestration *and* the holder side (admission, sibling rule, content-addressing, quota, store writes) now run as zero-authority JS in confined QuickJS realms over a generic capability bridge — the initiator in an async realm, the holder in a synchronous one. The Tier-2 guarantee holds end to end; an upgrade that keeps the same caps applies without re-granting trust.
