...
**The bundle — which carries the code, its names, and its authority.** Storage arrives as one author-signed bundle (seedkernel [§12.4](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md)), and that one signature is what authenticates storage's code to a node that may have received it through any number of relays — the one place signing genuinely survives. The bundle holds the pure-transform WASM modules (`codec`, `reputation`), declared in the manifest's signed `modules[]` and reached by the guest by their **bare name** — `host.call("codec", …)`. There is no bind name and no global namespace: a module is a private value of its own slot, and the seam resolves a bare name against that slot's map (seedkernel §5, §12.2), so nothing on the wire ever names another node's module. It also holds the confined guest program and, in the same signed manifest, the *reach* that guest may use (seedkernel §12.4), in two explicit, human-auditable lists declared by the author: the host services it is granted (`guest.requires` — `node`, `fs`, `clock`), gated **by service** by the host, and the co-resident guests it calls (`guest.calls` — `_net`), which carry no privilege at all. The pure modules — the codec (chunking, erasure coding) and the reputation math — hold **no** grants at all: they are computation, and the structural sandbox guarantees they can touch no I/O even if compromised. Deployments are kept apart not by names but by policy and scope: the admission policy's closed author set decides whose bundle may install (seedkernel [§12.5](https://github.com/arj03/seedkernel/blob/main/docs/RUNTIME.md)), and the scoped signature (§16) keeps one deployment's signed objects from verifying in another's.
...
| --- | --- | --- |
| `node` | granted service | the node's own things: `node/sign`/`node/verify` under **this slot's one scope** (§16's signing paragraph), `node/identity` (the node public key the channel pins), and `node/random` — the entropy source, used for content keys (nonces are derived, not drawn, §4.4) |
