...
Seedkernel routes a wire protocol id to one bundle's confined guest over an encrypted, authenticated channel; it has no notion of a file and nowhere for a multi-megabyte one to live. So bulk data lives beside its dispatch path, referenced by a content hash. Seed store is that outside store: the host keeps routing the protocol id to the slot's guest, the bytes never touch its dispatch path, and everything here is built from the pieces seedkernel already provides — a channel that pins who is speaking, a claim to dispatch on, and a signed bundle that carries the code and its authority.
...
**Design principles (inherited from seedkernel, applied to storage):**

- The storage layer adds **no new host concepts**. It is one signed bundle — pure-transform WASM modules + a confined guest — and a small message vocabulary, gated by the channel that already pins every peer and the bundle policy that already admits every bundle.
...
- **Identity comes from the channel.** A peer *is* the node public key its authenticated channel pinned (§2). Reciprocity and authority key on that one identity; nothing is signed to establish it.
