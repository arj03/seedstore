...
**The bundle — which carries the code, its names, and its authority.** Storage arrives as one author-signed bundle (seedkernel §12.4), and that one signature is what authenticates storage's code to a node that may have received it through any number of relays — the one place signing genuinely survives. The bundle holds the pure-transform WASM modules (`codec`, `reputation`), declared in the manifest's signed `modules[]` and reached by the guest by their **bare name** — `host.call("codec", …)`. There is no bind name and no global namespace: a module is a private value of its own slot, and the seam resolves a bare name against that slot's map (seedkernel §5.1, §12.2), so nothing on the wire ever names another node's module. It also holds the confined guest program and, in the same signed manifest, the *reach* that guest may use (`guest.requires`, seedkernel §12.4): an explicit, human-auditable list of host services (`node`, `fs`, `clock`) and local service ids (`_net`) declared by the author and gated **by service** by the shell — a `host.call` naming a method whose service is ungranted does not even resolve, and the loader refuses a `requires` entry naming a method rather than its service. The pure modules — the codec (chunking, erasure coding) and the reputation math — hold **no** grants at all: they are computation, and the structural sandbox guarantees they can touch no I/O even if compromised. Deployments are kept apart not by names but by policy and scope: the admission policy's closed author set decides whose bundle may install (seedkernel §12.5), and the scoped signature (§16) keeps one deployment's signed objects from verifying in another's.

**Crypto — reused, never shipped.** Storage reuses the kernel's cryptography rather than bundling its own, so the only cryptographic-grade algorithm in storage WASM is **Reed–Solomon** (libsodium has no erasure coding); hashing, stream encryption, and key-sealing are all calls over the same libsodium the kernel already loads — hashing and the stream cipher as the ungated `crypto/*` primitives of §16, key-sealing deliberately host-side (ibid.). Two consequences shape the rest of this doc: a chunk descriptor is signed and checked through the host's *scoped* signing pair (§16) — the host fixes the domain-and-scope prefix on both paths — never by crypto code shipped in storage WASM; and confidentiality *at rest*, which the channel's hop-by-hop encryption does not give (seedkernel §14), is added client-side over that same libsodium (§4.4). The payoff is storage modules that are logic + RS, tens of KB, with no second copy of a crypto library.
...
**Tier 1 — I/O base (trusted).** The shell's raw-byte capability backends — the flat-key `fs`, the `node` authorities (scoped sign/verify, identity, entropy), the clock, and the raw `link` seam the transport bundle owns — plus the ungated `crypto/*` primitive catalog over libsodium (seedkernel §12.1). These *must* be trusted: they are the only code that performs real I/O. They are deliberately small, each bound to exactly one service, audited, and change rarely. This is the *whole* of what seed store adds to your TCB.

**Tier 2 — logic (confined, upgradeable).** Everything else: `codec`, `reputation`, and the `coordinator`/`cohort`/`repair` orchestration (§17). This is pure logic. It is **confined** — a WASM module imports nothing, a realm has no ambient authority — and reaches I/O *only* through the Tier-1 services its manifest's `guest.requires` names. The grant is signed content, not something the logic could declare for itself: the author names the services in the signed bundle manifest (§2, seedkernel §12.4), and the shell wires only those backends — an ungranted service's names simply do not resolve in the realm. So upgrading the logic does *not* re-grant trust: a new version (same author, seedkernel §12.5) runs in the same sandbox, structurally bounded to whatever its manifest grants — and because the grant is signed, versioned content rather than ambient authority, a widened grant is a visible manifest change, never a silent acquisition.
...
- Generate a random per-file **content key** `K` and encrypt each chunk under `K` with the stream cipher (libsodium `crypto_stream_xchacha20_xor`), keystreamed under a 24-byte nonce of `level ∥ index` — the chunk's own signed index-tree level (§4.3), plus its chunk index within that level. The cipher is length-preserving, so the ciphertext is the same size as the plaintext: slice it into `B`-byte blocks and erasure-code, with no tag to misalign them. A fresh per-file random `K` already makes every `(K, nonce)` pair unique — a different file draws a different `K`, and within a file the index never repeats — and `(K, nonce)` reuse is the one thing to avoid, since reuse collapses a stream cipher to a two-time pad and forfeits confidentiality outright. Holders store ciphertext blocks and learn nothing about content. The index levels (§4.3) are encrypted under `K` the same way — the level byte is what keeps an index chunk's keystream disjoint from the body chunk of the same index, so there is no separate manifest domain to keep in step; key rotation (§23) mints a fresh `K`.
...
1. **Resolve the root.** You were handed the root descriptor and the sealed `K`. If its signed `level` is 0 it already describes file bytes and you are done. Otherwise read that chunk by steps 2–4 below — the same fetch, verify, decode, decrypt — and its plaintext *is* the descriptor list one level down; repeat, which in practice means once. **Each step must strictly decrease `level`**, and a reader enforces that rather than assuming it: whoever shared the file chose the plaintext at every level (§4.4's cipher has no tag), so nothing else stops a level-ℓ root from naming a level-ℓ list — the bytes really do hash to their ids and the descriptors really are signed. Without the check a reader can be walked through an arbitrarily long non-descending chain, built bottom-up at no cost to the sharer. With it, the descent is bounded by the `level` byte. The file size is the sum of the leaves' signed `tail_bytes`. No *signature* check is needed here: the root is in your hand and everything under it is content-addressed (§4.2).
...
Storage's confined logic reaches the world through one seam — `host.call(name, bytes)` (seedkernel §12.2) — and the names on it that storage relies on come in three kinds: **granted host services** (real I/O or the node's authority, each the shell's own), one **local service id** (`_net`, another bundle's — the transport's — with no privilege of its own), and **ungated names** — the `crypto/*` primitive catalog and the bundle's own module names — which reach nothing a guest could not compute itself and therefore carry no grant at all. Together the granted ones are the whole of Tier 1 (§2.1). They are not storage-specific kernel objects: they are the kernel's own surface (seedkernel §12.1–§12.2), and an undeclared service's method names simply do not resolve in the realm (§2.1).
...
| `crypto/blake2b-256`, `crypto/xchacha20/xor` | ungated primitive | the §4.2 content hash and the §4.4 length-preserving stream cipher |
| *bare names* — `codec`, `reputation` | the bundle's own pure-transform modules | ungated for the same reason as the primitives: they are the bundle's own verified bytes — the scope of one app's map is the shape, not a grant (seedkernel §12.2) |

**Grants are by service, not by method.** A manifest's `guest.requires` names `fs`, never `fs/get`; the shell gates a `host.call` by the method's service, and the loader refuses a `requires` entry naming a method (`fs/get`) rather than its service. A name whose service is undeclared is refused at load; a `host.call` naming something the realm never gained simply does not resolve — the backend is not even wired behind the check (seedkernel §12.2).

`_net` is the one thing on the list that is not the shell's: it is ordinary claim routing (seedkernel §12.10). The transport is itself a bundle, holding the **`link`** capability — the one privileged raw-net seam, granted by the operator's policy (§12.5) — and showing a structured service under a local service id the storage guest names in its `requires`. Calling `_net` carries no privilege: it is a co-resident call from one bundle slot to another, and the shell reaches it through the same claim table it uses for a peer's protocol id. `clock` and the entropy source are conventional backends the deployment already has; storage adds none.

**Crypto is reused, not added.** The two primitives storage uses are the kernel's own catalog entries (seedkernel §12.1): `crypto/blake2b-256` (the `content_hash` for block-ids) and `crypto/xchacha20/xor` (length-preserving encryption under a content key; the same op decrypts). They perform no I/O, so they need **no grant**, which is what lets `codec` stay pure (§17). **Why a storage-local hash:** block-ids never cross into the kernel — they are pure content addressing within storage (§4.2) — so the content hash is storage's own choice rather than something the kernel imposes, and **BLAKE2b-256** is fast and *also* already in this same libsodium, so it adds no bytes. That choice and the kernel's genesis hash now happen to *coincide* — seedkernel standardized on BLAKE2b-256 as its one system hash (§5.1) — but they remain separately decided: storage would keep BLAKE2b block-ids whatever the kernel hashed manifests with. Key-sealing (*sealing* `K` to a recipient's kernel key, §4.4) is deliberately absent from the seam — the kernel keeps `crypto_box_seal` above no target (seedkernel §14.1) — so the one audited use lives beside the guest in the reference implementation (`host/crypto.ts`, over the same shared sumo libsodium), never in signature-bearing storage code. Storage therefore ships **no crypto of its own except Reed–Solomon** (libsodium has no erasure coding), reusing the same libsodium the deployment already carries rather than bundling a second copy — and the one symbol it needs in a nonstandard form, the MAC-less `crypto_stream_xchacha20_xor` (§4.4), is exactly why the shell itself already loads the **sumo** build (seedkernel §12.1), so no storage-specific crypto packaging exists at all.

**The sign/verify pair exists — but it is scoped, and every signed format is tagged.** Storage logic authors its signed objects — the chunk descriptor (§4.3) and the optional tombstone (§25) and file head (§27.3) — through the kernel's scoped pair, never a raw oracle (seedkernel §12.2): `node/sign` signs `DOMAIN_guest ‖ scope ‖ msg` where the scope is the slot's own, derived from the admitted bundle's `(author, app)` — never from the caller — and **verification is scoped identically under the same name**, so storage names the verifying key and never reconstructs the prefix; the host applies it on both paths. (The raw `crypto/ed25519/verify` primitive stays, ungated like every pure transform, for bytes a caller holds from another context — storage never hands it one.) A storage signature thus verifies only as a storage signature — never as a bundle manifest or a channel handshake, the disjoint members of the kernel's domain family (seedkernel §16.1, §14) — and never as another app's object. The signatures stay portable across the cohort because every node running the same bundle derives the same scope, and checking one still needs only the signer's public key — keyless repair (§9) is untouched. One level down, seed store applies the kernel's sub-separation rule to its own vocabulary: each signed storage format opens with a **distinct leading tag** — one for the descriptor, one for the tombstone, one for the head — so a signed object of one type can never be replayed as another. Signing never ships in storage WASM at all — a module is a pure transform with no grants (seedkernel §4.2, §12.2); what storage holds is the seam's scoped pair, never key material.
...
| --- | --- | --- | --- |
| `codec` | WASM module | — (pure) | slice into `B`-byte blocks; encrypt/decrypt with the length-preserving stream cipher, compute block-ids via the ungated `crypto/blake2b-256` primitive, seal/unseal content keys client-side (§16); **Reed–Solomon encode/decode (on ciphertext) — the only algorithm in-WASM**; build/parse chunk descriptors |
