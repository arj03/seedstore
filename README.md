...
- **Block** — the one unit of stored data: a content-addressed slice of ciphertext of `B` bytes (block size `B` is a deployment tuning knob, §27, kept small so a whole block re-fetches cheaply, §8; nothing ties it to the 64 KB envelope — bulk blocks aren't enveloped, §3). `block_id = genesis_hash(block_bytes)`. A block is simultaneously an erasure-coding shard *and* the unit that moves on the wire.
...
**Bulk plane — content-addressed blocks.** Blocks are **self-verifying by hash**, so they need no signature for integrity. In the base design they ride the existing data channel as **unsigned, hash-verified frames** — each frame is `[claimed block_id ∥ bytes]`, the claimed id a routing label only: the receiver drops any frame whose claimed id it did not request — so unsolicited bulk costs nothing, not even a hash — then verifies `genesis_hash(bytes) == claimed id` on the rest (the hash, never the label, is the authority) and drops on mismatch. This stays entirely inside the seedkernel message model, inherits the channel's encryption and pubkey pinning, and avoids per-block verify cost because there is no signature to check. (A dedicated bulk data channel for higher throughput on large files is an optional upgrade — §22.)
...
- **Lost** — unreachable beyond `G`, or repeatedly failing to serve a block it advertises. Eligible to be counted as missing for repair, once the unreachability is corroborated beyond a single observer so that an eclipse of one holder does not by itself spark a repair storm.
...
3. **Avoid duplicate work** with a jittered timer: the peer that fires first announces it (`repair.claim`); others hold off and cancel when a freshly placed block shows up in have/want. A claim is a **promise**: if no fresh block appears within a bounded deadline the others resume regardless, the silent claimer's standing decays as for a missed verification-fetch (§13), and a recent defaulter's claims are ignored — so a peer cannot grief repair by claiming chunks it never heals. Because the cohort is small and both the claim and its (non-)fulfilment are observable, this needs no election or coordinator.
...
**Crypto is reused, not added.** Hashing and the §4.4 confidentiality primitives are thin host services over the **libsodium the kernel already loads for its genesis suite** (seedkernel §6.2, §11.2): `hash` (SHA-3-256 — the `genesis_hash` used for block-ids), `stream` (length-preserving encryption via `crypto_stream_xchacha20_xor` under a content key; the same op decrypts), and `box_seal`/`box_open` (sealing `K` to a recipient's kernel key). These perform no I/O, so — like `signature.signer` — they need **no capability**, which is what lets `codec` stay pure (§17). Signing stays sender-side in the host (seedkernel §6.1), so a descriptor is authored by wrapping it as a signed envelope (§4.3), never by a signing call from WASM. Storage therefore ships **no crypto of its own except Reed–Solomon** (libsodium has no erasure coding), reusing the ~217 KB libsodium the deployment already carries rather than bundling a second copy.
